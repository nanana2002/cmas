...
import (
    ...
    "strconv"
    "strings"
    "time"  // 新增：必须导入time包才能使用time.Second
)
...
func getNetworkDelay(csciID string) int {
    ...
    if ip == "" {
        logger.Warn("CPS", "提取IP失败，使用兜底延迟5ms")
        return 5
    }
    ...
    if err != nil {
        logger.Warn("CPS", "ping %s 失败：%v，使用兜底延迟5ms", ip, err)
        return 5
    }
    ...
    if len(matches) == 0 {
        logger.Warn("CPS", "未匹配到%s的延迟值，使用兜底延迟5ms", ip)
        return 5
    }
    ...
    logger.Info("CPS", "IP %s 的平均延迟：%dms", ip, avgDelay)
    return avgDelay
}

// 检测Site是否可用
func isSiteAvailable(csciID string) bool {
    // 正确写法：time.Second（导入time包后）
    client := &http.Client{Timeout: 2 * time.Second}
    resp, err := client.Get("http://" + csciID + "/metrics")
    if err != nil || resp.StatusCode != 200 {
        logger.Warn("CPS", "Site %s 不可用", csciID)
        return false
    }
    defer resp.Body.Close()
    logger.Debug("CPS", "Site %s 可用", csciID)
    return true
}

// 从C-SMA获取聚合指标
func getCmaMetrics() (map[string][]models.ServiceInstanceInfo, error) {
    logger.Debug("CPS", "请求C-SMA聚合指标：%s/sync", config.Cfg.CSMA.URL)
    resp, err := http.Get(config.Cfg.CSMA.URL + "/sync")
    if err != nil {
        logger.Error("CPS", "获取C-SMA指标失败：%v", err)
        return nil, err
    }
    defer resp.Body.Close()
    ...
    if err := json.NewDecoder(resp.Body).Decode(&cmaResp); err != nil {
        logger.Error("CPS", "解析C-SMA指标失败：%v", err)
        return nil, err
    }
    logger.Info("CPS", "获取C-SMA指标成功，共%d个服务", len(cmaResp.Data))
...
func main() {
    ...
    r.POST("/select", func(c *gin.Context) {
        ...
        if err := c.ShouldBindJSON(&req); err != nil {
            logger.Error("CPS", "参数解析失败：%v", err)
            c.JSON(http.StatusBadRequest, gin.H{"success": false, "msg": err.Error()})
            return
        }
        logger.Info("CPS", "接收选择请求，服务ID：%s，最大成本：%d，最大延迟：%d", req.ServiceID, req.MaxAcceptCost, req.MaxAcceptDelay)

        // 1. 获取C-SMA指标
        metrics, err := getCmaMetrics()
        if err != nil {
            logger.Error("CPS", "获取指标失败：%v", err)
        ...
        for _, instance := range metrics[req.ServiceID] {
            ...
            if instance.Cost <= req.MaxAcceptCost && instance.Gas > 0 {
                ...
                logger.Debug("CPS", "候选Site：%+v", instance)
            }
        }

        // 3. 无可用实例
        if len(candidates) == 0 {
            logger.Warn("CPS", "无符合条件的服务实例，服务ID：%s", req.ServiceID)
        ...
        // 5. 返回最优Site
        best := candidates[0]
        logger.Info("CPS", "最优Site：%s，成本：%d，延迟：%d", best.CSCIID, best.Cost, best.Delay)
